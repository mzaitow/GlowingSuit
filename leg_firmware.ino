...
{      
  ...
  Wire.begin();
  FastLED.addLeds<NEOPIXEL, 2>(leds2, NUM_LEDS);  // GRB ordering is assumed
  FastLED.addLeds<NEOPIXEL, 3>(leds1, NUM_LEDS);
...
void led_strip_1_effect(float angle) {
  int i = 0;
  int peek = (abs((int)angle) * NUM_LEDS) / 90;
...
void led_strip_2_effect(float angle) {
  int i = 0;
  int peek = (abs((int)angle) * NUM_LEDS) / 90;
...
void loop()
{
  ...
  led_strip_1_effect(angle_y - 20);
  led_strip_2_effect(angle_y - 20);
  FastLED.show();
}
